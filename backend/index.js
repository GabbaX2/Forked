...
// Aggiungi ricetta - FIXED: changed from recipies to recipes for consistency
...
// Visualizza ricette dell'utente loggato - FIXED: changed from myrecipies to myrecipes
...
// Generazione lista della spesa - POST
app.post('/forked/lista-spesa', async (req, res) => {
    try {
        ...
        const ricetteIds = ricette.map(id => {
            if (!ObjectId.isValid(id)) {
                throw new Error(`ID ricetta non valido: ${id}`);
            }
        ...
        ricetteSelezionate.forEach(ricetta => {
            if (ricetta.ingredients && Array.isArray(ricetta.ingredients)) {
                ricetta.ingredients.forEach(ingrediente => {
                    ...
                    listaSpesa[key].quantita += (ingrediente.quantita || 0) * persone;
                });
            }
        });
        ...
        res.status(500).json({ message: 'Errore nella generazione della lista: ' + error.message });
    }
});
...
app.get('/forked/lista-spesa', async (req, res) => {
    try {
        ...
        ricetteSelezionate.forEach(ricetta => {
            if (ricetta.ingredients && Array.isArray(ricetta.ingredients)) {
                ricetta.ingredients.forEach(ingrediente => {
                    ...
                    listaSpesa[key].quantita += (ingrediente.quantita || 0) * numPersone;
                });
            }
        });
        ...
        console.error('Errore lista spesa GET:', error);
        res.status(500).json({ message: 'Errore nella generazione della lista: ' + error.message });
    }
});
